...
namespace Libraries::Hash
{
	...
	MD5::transform (const uint8_t * block) noexcept
	{
		...
		this->decode(hash.data(), block, MD5::BlockSize);
	...
	MD5::update (const uint8_t * message, size_t length) noexcept
	{
		...
		if ( length >= firstPart )
		{
			...
			memcpy(&m_buffer[index], message, firstPart); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
		...
		memcpy(&m_buffer[index], &message[msgIndex], length - msgIndex); // NOLINT(cppcoreguidelines-pro-bounds-constant-array-index)
	}
	...
	MD5::final (std::array< uint8_t, 16 > & digest) noexcept
	{
		...
		this->encode(bits.data(), m_count.data(), bits.size());
		...
		this->encode(digest.data(), m_states.data(), digest.size());
