/*
 * Libraries/PixelFactory/FileIO.hpp
...
{
	/** @brief Enumerates pixmap file type handled. */
	enum class FileType
	{
		Unhandled,
		Jpeg,
	...
	[[nodiscard]]
	bool
	read (const Path::File & filepath, Pixmap< data_t > & pixmap)
	{
		if ( !filepath.exists() )
		{
	...
	[[nodiscard]]
	bool
	write (const Pixmap< data_t > & pixmap, const Path::File & filepath, bool overwrite = false)
	{
