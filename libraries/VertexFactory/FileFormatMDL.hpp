/*
 * Libraries/VertexFactory/FileFormatMDL.hpp
...
{
	// NOLINTBEGIN(readability-identifier-length, cppcoreguidelines-pro-type-reinterpret-cast, *-magic-numbers)
	...
	class FileFormatMDL final : public FileFormatInterface< data_t >
	{
		public:
			...
			{
				std::ifstream file(to_string(filepath), std::ios::in | std::ios::binary);
				...
				if ( m_flags[UseMDLNormal] )
				{
					...
					for ( size_t triangleIndex = 0; triangleIndex < triangleCount; triangleIndex++ )
					{
						for ( size_t vertexIndex = 0; vertexIndex < 3UL; vertexIndex++ )
						{
							...
							builder.newVertex(Math::Vector< 3, data_t >{
	...
	// NOLINTEND(readability-identifier-length, cppcoreguidelines-pro-type-reinterpret-cast, *-magic-numbers)
}
