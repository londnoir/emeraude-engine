/*
 * Libraries/VertexFactory/Silhouette.hpp
...
namespace Libraries::VertexFactory
{
	...
	class [[maybe_unused]] Silhouette final
	{
		public:
			 ...
			 */
			bool
			build (const Shape< data_t > & geometry, const Math::Coordinates< data_t > & pointOfView) noexcept
			{
				if ( geometry.isValid() )
				{
				...
				for ( const auto & triangle : triangles )
				{
					...
					if ( Math::Vector< 3, data_t >::dotProduct(triangle.surfaceNormal(), direction) > 0.0F )
					{
