...
namespace Libraries
{
	...
	class PerlinNoise final
	{
		public:
			...
			[[nodiscard]]
			type_t
			generate (type_t x, type_t y, type_t z) noexcept
			{
				...
				auto A = m_permutations[X] + Y;
				auto AA = m_permutations[A] + Z;
				auto AB = m_permutations[A + 1] + Z;
				auto B = m_permutations[X + 1] + Y;
				auto BA = m_permutations[B] + Z;
				auto BB = m_permutations[B + 1] + Z;

				/* Adds blended results from 8 corners of cube. */
				auto res = this->linearInterpolation(
						w,
				...
				return (res + static_cast< type_t >(1.0)) * static_cast< type_t >(0.5);
			}
			...
			[[nodiscard]]
			uint8_t
			generate8bits (type_t x, type_t y) noexcept
			{
				return static_cast< uint8_t >(static_cast< type_t >(256.0) * this->generate(x, y, 0));
			}
		...
		private:
			 ...
			 */
			type_t
			linearInterpolation (type_t t, type_t a, type_t b) noexcept
			{
				return a + t * (b - a);
			}
			 ...
			 */
			type_t
			grad (unsigned int hash, type_t x, type_t y, type_t z) noexcept
			{
				auto h = hash & 15;
