/*
 * Libraries/Math/OrientedCuboid.hpp
...
namespace Libraries::Math
{
	...
	class OrientedCuboid : public Shape3DInterface< data_t >
	{
		public:
			 ...
			 */
			OrientedCuboid (const Cuboid< data_t > & cuboid, const Coordinates< data_t > & coordinates) noexcept
			{
			 ...
			 */
			bool
			set (const Cuboid< data_t > & cuboid, const Coordinates< data_t > & coordinates) noexcept
			{
				if ( !cuboid.isValid() )
				{
					return false;
				...
				const auto matrix = coordinates.modelMatrix();
			 ...
			 * @param cuboidB A reference to an other orientedCuboid.
			...
			isIntersecting (const OrientedCuboid< data_t > & cuboidA, const OrientedCuboid< data_t > & cuboidB, Vector< 3, data_t > & direction) noexcept
			{
				...
				for ( const auto & axis : cuboidA.m_normals )
				{
					...
					auto projectionA = cuboidA.project(axis);
					auto projectionB = cuboidB.project(axis);
					...
					/* Gets the overlap. */
					auto overlap = projectionA.getOverlap(projectionB);

					/* Checks for minimum overlapping. */
					if ( overlap > 0 && overlap < distance )
					{
						/* If minimum, we set this one as the smallest. */
						distance = overlap;
						direction = axis;
					}
				}
