...
namespace Libraries::Math
{
	...
	>
	class [[maybe_unused]] BSpline final
	{
		public:
			...
			setDefaultSegments (size_t defaultSegments) noexcept
			{
				...
				{
					std::cerr << __PRETTY_FUNCTION__ << ", default segment should at least be 1 !" << std::endl;
			 ...
			 * @return BSplinePoint< type_t, dim_t > &
			 */
			inline
			BSplinePoint< type_t, dim_t > &
			...
			inline
			BSplinePoint< type_t, dim_t > &
			addPoint (const Vector< dim_t, type_t > & position, const Vector< dim_t, type_t > & handle) noexcept
			{
			...
			inline
			BSplinePoint< type_t, dim_t > &
			addPoint (const Vector< dim_t, type_t > & position, const Vector< dim_t, type_t > & handleIn, const Vector< dim_t, type_t > & handleOut) noexcept
			{
		...
		private:
			 ...
			 */
			bool
			synthesizeLinear (const Callback & callback, const BSplinePoint< type_t, dim_t > & currentPoint, const BSplinePoint< type_t, dim_t > & nextPoint, float & currentTime, float timeStep) const noexcept
			{
			 ...
			 */
			bool
			synthesizeQuadratic (const Callback & callback, const BSplinePoint< type_t, dim_t > & currentPoint, const BSplinePoint< type_t, dim_t > & nextPoint, float & currentTime, float timeStep) const noexcept
			{
				/* Time step along a segment. */
				const auto factorStep = 1.0F / currentPoint.segments();
				...
				for ( size_t segment = 0; segment < currentPoint.segments(); segment++ )
				{
					...
					if ( !callback(currentTime, Vector< dim_t, type_t >::quadraticBezierInterpolation(currentPoint.position(), handleOut, nextPoint.position(), factor)) )
					{
						return false;
					}
			 ...
			 */
			bool
			synthesizeCubic (const Callback & callback, const BSplinePoint< type_t, dim_t > & currentPoint, const BSplinePoint< type_t, dim_t > & nextPoint, float & currentTime, float timeStep) const noexcept
			{
				/* Time step along a segment. */
				const auto factorStep = 1.0F / currentPoint.segments();
				...
				for ( size_t segment = 0; segment < currentPoint.segments(); segment++ )
				{
					...
					if ( !callback(currentTime, Vector< dim_t, type_t >::cubicBezierInterpolation(currentPoint.position(), handleOut, handleIn, nextPoint.position(), factor)) )
					{
						return false;
					}
			...
			std::vector< BSplinePoint< type_t, dim_t > > m_points{};
