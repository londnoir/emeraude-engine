...
namespace EmEn::Libs::PixelFactory
{
	...
	class Font final
	{
		public:
			...
			{
				const auto isColumnEmpty = [&glyph] (size_t coordX)
				{
				...
				size_t width = glyph.width() - offset;

				for ( size_t coordX = glyph.width(); coordX-- > 0UL; )
				{
					if ( !isColumnEmpty(coordX) )
					{
						break;
				...
				return {offset, 0UL, width, glyph.height()};
			}
