...
namespace EmEn::Libs::Utility
{
	...
	[[nodiscard]]
	float_t
	round (float_t value, size_t precision = 0) requires (std::is_floating_point_v< float_t >)
	{
	 ...
	 * @warning This version use the unreliable old C rand() function. Use EmEn::Libs::Randomizer class instead.
	...
	[[nodiscard]]
	type_t
	quickRandom (type_t min, type_t max) requires (std::is_integral_v< type_t >)
	{
		if ( min > max )
		{
			std::swap(min, max);
	...
	[[nodiscard]]
	type_t
	quickRandom (type_t min, type_t max) requires (std::is_floating_point_v< type_t >)
	{
		if ( max < min )
		{
			std::swap(min, max);
	...
	[[nodiscard]]
	bool
	isOne (float_t value, float_t epsilon = std::numeric_limits< float_t >::epsilon()) noexcept requires (std::is_floating_point_v< float_t >)
	{
		return equal(value, static_cast< float_t >(1), epsilon);
	}
