...
namespace EmEn::Libs::Math::Space2D
{
	...
	/**
	 * @brief Checks if a line is intersecting a circle and gives the intersection point.
	 * @tparam precision_t The data precision. Default float.
	 * @param line A reference to a line.
	 * @param Circle A reference to a circle.
	...
	static
	bool
	isIntersecting (const Line< precision_t > & line, const Circle< precision_t > & Circle, Point< precision_t > & intersection) noexcept
		requires (std::is_floating_point_v< precision_t >)
	{
		// TODO...
	...
	/** @copydoc EmEn::Libs::Math::Space2D::isIntersecting(const Line< precision_t > &, Circle< precision_t > &, Point< precision_t > &) noexcept */
	template< typename precision_t = float >
	[[nodiscard]]
	static
	bool
	isIntersecting (const Circle< precision_t > & Circle, const Line< precision_t > & line, Point< precision_t > & intersection) noexcept requires (std::is_floating_point_v< precision_t >)
	{
		return isIntersecting(line, Circle, intersection);
	}
}
