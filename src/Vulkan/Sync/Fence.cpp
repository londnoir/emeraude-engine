/*
 * Emeraude/Vulkan/Sync/Fence.cpp
...
namespace Emeraude::Vulkan::Sync
{
	...
	Fence::createOnHardware () noexcept
	{
		...
		TraceSuccess{ClassId} << "The fence " << m_handle << " (" << this->identifier() << ") is successfully created !";
	...
	Fence::destroyFromHardware () noexcept
	{
		...
		if (  m_handle != VK_NULL_HANDLE )
		{
			...
			TraceSuccess{ClassId} << "The fence " << m_handle << " (" << this->identifier() << ") is gracefully destroyed !";
	...
	VkFence
	Fence::handle () const noexcept
	{
		return m_handle;
	}

	VkFenceCreateInfo
	Fence::createInfo () const noexcept
	{
		return m_createInfo;
	}
