/*
 * Emeraude/Graphics/Light/PointArrayBuffer.hpp
...
#include "Scenes/PointLight.hpp"

namespace Emeraude::Graphics::Light
{
	/**
	* @brief The point light array buffer class. This will manage the GPU buffer to set light information to shaders.
	* @extends Emeraude::Graphics::ArrayBufferInterface The base class of light array buffer.
	*/
	class PointArrayBuffer final : public ArrayBufferInterface
	{
		public:

			static constexpr auto MaxActivableLight = 3UL;
			 ...
			 */
			explicit PointArrayBuffer (size_t maxLight, size_t HWAlignmentSize = 256) noexcept;
			 ...
			 * @return bool
			 */
			bool addLight (const std::shared_ptr< Scenes::PointLight > & light) noexcept;

			/**
			 * @brief Removes a new light from the buffer.
			 * @param light The light to delete.
			 */
			void removeLight (const std::shared_ptr< Scenes::PointLight > & light) noexcept;

			/**
			 * @brief Returns a lights list.
			 * @return const vector< shared_ptr< PointLight > &
			 */
			[[nodiscard]]
			const std::vector< std::shared_ptr< Scenes::PointLight > > & lights () const noexcept;

		private:

			std::unique_ptr< Vulkan::UniformBufferObject > m_uniformBufferObject{};
			std::vector< std::shared_ptr< Scenes::PointLight > > m_lights{};
			size_t m_limit = 64;
