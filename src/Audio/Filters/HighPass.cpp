/*
 * Emeraude/Audio/Filters/HighPass.cpp
...
namespace Emeraude::Audio::Filters
{
	...
	HighPass::setGain (float value) noexcept
	{
		...
		{
			Tracer::warning(ClassId, Blob() << "Gain must be between " << AL_HIGHPASS_MIN_GAIN << " and " << AL_HIGHPASS_MAX_GAIN << ".");
	...
	HighPass::setGainLF (float value) noexcept
	{
		...
		{
			Tracer::warning(ClassId, Blob() << "Gain must be between " << AL_HIGHPASS_MIN_GAINLF << " and " << AL_HIGHPASS_MAX_GAINLF << ".");
