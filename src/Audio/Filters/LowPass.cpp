/*
 * Emeraude/Audio/Filters/LowPass.cpp
...
namespace Emeraude::Audio::Filters
{
	...
	LowPass::setGain (float value) noexcept
	{
		...
		{
			Tracer::warning(ClassId, Blob() << "Gain must be between " << AL_LOWPASS_MIN_GAIN << " and " << AL_LOWPASS_MAX_GAIN << ".");
	...
	LowPass::setGainHF (float value) noexcept
	{
		...
		{
			Tracer::warning(ClassId, Blob() << "Gain must be between " << AL_LOWPASS_MIN_GAINHF << " and " << AL_LOWPASS_MAX_GAINHF << ".");
