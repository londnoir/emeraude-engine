/*
 * src/Libraries/PixelFactory/Area.hpp
...
namespace Libraries::PixelFactory
{
	...
	class Area final
	{
		public:
			...
			/**
			 * @brief operator + overload.
			 * @param rhs A reference to an area.
			 * @return bool
			 */
			[[nodiscard]]
			Area &
			...
			[[nodiscard]]
			Area &
			operator+= (const Area & rhs) noexcept
			{
			 ...
			 * @tparam output_t The type of number. Default float.
			 * @return Vector< 2, output_t >
			 */
			template< typename output_t = float >
			[[nodiscard]]
			Math::Vector< 2, output_t >
			bottomLeft () const noexcept requires (std::is_arithmetic_v< output_t >)
			{
			...
			[[nodiscard]]
			Math::Vector< 2, output_t >
			bottomRight () const noexcept requires (std::is_arithmetic_v< output_t >)
			{
			...
			[[nodiscard]]
			Math::Vector< 2, output_t >
			topLeft () const noexcept requires (std::is_arithmetic_v< output_t >)
			{
			...
			[[nodiscard]]
			Math::Vector< 2, output_t >
			topRight () const noexcept requires (std::is_arithmetic_v< output_t >)
			{
